...
--
------------------------------------------
local function GetIsUseOrder(orderType)
  return orderType == kTechId.Construct
    or orderType == kTechId.AutoConstruct
    or orderType == kTechId.Build
    or orderType == kTechId.Weld
end
...
kMarineBrainActions =
  {
    ...
    function(bot, brain)
      ...
      local marine = bot:GetPlayer()
      local sdb = brain:GetSenses()
--      Print("------- getting biggestThreat")
      local threat = sdb:Get("biggestThreat")
      local weight = 0.0

      if threat ~= nil and sdb:Get("weaponReady") then

--        local threatEntity = Shared.GetEntity(threat.memory.entId)
--
--        if threatEntity then
--          name = name..threatEntity:GetClassName()
--        end
    ...
    function(bot, brain)
      ...
      local nearestWeldable = sdb:Get("nearestWeldable")
      if (sdb:Get("welder") ~= nil) and (nearestWeldable ~= nil) and (nearestWeldable.GetWeldPercentage ~= nil) and (nearestWeldable:GetWeldPercentage() < 1) then
        weight = 2.0
      else
        weight = 0.0
      end
      ...
      return { name = name, weight = weight,
        perform = function(move)
          ...
          if not sdb:Get("welderReady") then
            ...
            PerformUse( marine, nearestWeldable, bot, brain , move )
          end
    ...
    function(bot, brain)
      ...
      local marine = bot:GetPlayer()
      local botId = marine:GetId()
      local order = bot:GetPlayerOrder()
      local orderType = nil
      local teamBrain = brain:GetTeamBrain()
      local targetId = nil
      local target = nil
      ...
      if order ~= nil then
--        weight = brain:GetCurrentOrderWeight()
        weight = 3.0
        orderType = order:GetType()
        targetId = order:GetParam()
        target = Shared.GetEntity(targetId)

        if target then
          if orderType == kTechId.Construct or orderType == kTechId.Build then

            -- Because construct orders are often given by the auto-system, do not necessarily obey them
            -- Load-balance them
            local numOthers = teamBrain:GetNumOthersAssignedToEntity(targetId, botId)
            if numOthers >= 1 then
              weight = 0.0
            end
          elseif HasMixin(target, "Weldable") and (target:GetWeldPercentage() < 1) then
            local numOthers = teamBrain:GetNumOthersAssignedToLeaderId(targetId, botId)
            if numOthers >= 2 then
              weight = 0.0
            else
              weight =EvalLPF(target:GetWeldPercentage(), {
                { 0.0, 10.0 },
                { 0.9, 3.0 },
                { 1.0, 3.0 }
              })
      ...
      return { name = name, weight = weight,
        perform = function(move)
          if order then
            ...
            local isUseOrder = GetIsUseOrder(orderType)
            local targetTeammable = HasMixin(target, "Team")
            local teamNumber = marine:GetTeamNumber()
            local targetIsAlly = targetTeammable and (target:GetTeamNumber() == teamNumber)                    
            local targetIsEnemy = targetTeammable and (target:GetTeamNumber() ~= teamNumber)
            local targetWeldable = HasMixin(target, "Weldable") 
            -- если это противник                   
            if targetIsEnemy then
              -- если команда - атака и можно атаковать, атакуем непосредственно эту цель
              if (orderType == kTechId.Attack) and HasMixin(target, "Target")  then
                PerformAttackEntity( marine:GetEyePos(), target, order:GetLocation(), bot, brain, move )
              --elseif orderType == kTechId.Move then
              -- иначе просто идем к ней
              else
                PerformMove(marine:GetOrigin(), order:GetLocation(), bot, brain, move)
              end
            elseif targetIsAlly then
              local periodKoeff = 1
              if orderType == kTechId.Defend
                 or orderType == kTechId.Follow
                 or orderType == kTechId.AutoWeld
                 or orderType == kTechId.Default
                then
                periodKoeff = 3
              elseif orderType == kTechId.SetTarget then
                periodKoeff = 0.3
              elseif isUseOrder then
                periodKoeff = 0
                PerformUse(marine, target, bot, brain, move)
              end
              if (periodKoeff > 0) and not brain.teamBrain:GetBotIdFollowsLeaderId(botId, targetId) then
                local periodSec = 120.0 * brain.characterTraits.diligence * periodKoeff
                teamBrain:AssignBotIdToLeaderId(marine:GetId(), destId)
                brain.endFollowingLeaderTime = Shared.GetTime() + periodSec
--               PrintToChat(marine, false, string.format("I follow for %.1f seconds, ends at %s", periodSec, os.date("%c", brain.endFollowingLeaderTime)))
                if targetWeldable and (target:GetWeldPercentage() < 1) then
                  PerformUse(marine, target, bot, brain, move)
                else
                  PerformMove(marine:GetOrigin(), order:GetLocation(), bot, brain, move)
                end
    ...
    function ( bot, brain )
      ...
      local minDist = 0
      local needUse = false
      if (brain.endFollowingLeaderTime) and (Shared.GetTime() > brain.endFollowingLeaderTime) then
        brain.endFollowingLeaderTime = nil
        teamBrain:UnassignBotByIdFromFollowers(marine:GetId())
      elseif teamBrain then
        local leaderId = teamBrain:GetBotLeaderId(marine:GetId())
        if leaderId then
          leader = Shared.GetEntity(leaderId)
          if leader then
            distance = marine:GetOrigin():GetDistance(leader:GetOrigin())
            minDist = 5.0
            if HasMixin(leader, "Weldable") and (leader:GetWeldPercentage() < 1) then
              needUse = true
              weight = 2.0
            else
              weight = EvalLPF(distance, {
                { 0.0, 2.0 },
                { 20, 1.0 },
                { 100, 0.1 }
              })
      ...
      return { name = name, weight = weight,
        perform = function(move)
--          Print("PERFORM FOLLOW TO LEADER === === ===")
          if needUse then
            PerformUse(marine, leader, bot, brain, move)
          else
            PerformMove(marine:GetOrigin(), destPoint, bot, brain, move )
          end
        end
      }
    end,
    ...
    function(bot, brain)
      ...
      local weaponTechs = {
        [kTechId.WelderTech] = kTechId.Welder,
...
local function GetAttackUrgency(bot, mem)
  ...
  local passiveUrgencies =
    {
      [kMinimapBlipType.Crag] = numOthers >= 4           and 0.2 or 0.95, -- kind of a special case
      ...
      -- todo: проверять, близко ли хайв - если нет, то высокий уровень
