        ...
        return function (playerData)
            ...
            if obj then
                ...
                if gMainMenu then
                    ...
                    gMainMenu.rankLevel:SetText(string.format( Locale.ResolveString("MENU_LEVEL"), obj.level ))                    
                    gMainMenu.rankLevelBar:SetText(string.format( Locale.ResolveString("MENU_NEXT_LEVEL"), obj.level + 1))
                    
                    local xpEarned, xpNeeded = obj.xp - PlayerRanking_GetTotalXpNeededForLevel( obj.level ), PlayerRanking_GetXPNeededForLevel(obj.level + 1)
                    gMainMenu.rankLevelBar.levelBar:SetValue( Clamp( xpEarned / xpNeeded, 0, 1 ) )

                    local rookie = gMainMenu.playerLevel < kRookieAllowedLevel
...
function GUIMainMenu:CreateMainLink(text, linkNum, OnClick)
    ...
    if text == "MENU_TRAINING" then
        ...
        if not Client.GetAchievement('First_0_9') then
            ...
            mainLinkGlow.topOffsetOriginal = 40 + 70 * linkNum
            ...
            mainLinkAlertTextGlow.topOffsetOriginal = 40 + 70 * linkNum
...
function GUIMainMenu:CreateProfile()
    ...
    --TODO Add Tooltip for progressbar. Explain scoring. Link to Hive?
    ...
    self.rankLevelBarBorder.hiveLink:AddEventCallbacks(eventCallbacks)
...
function GUIMainMenu:ProcessJoinServer(pastWarning)
    if MainMenu_GetSelectedServer() then
        ...
        then
            ...
            local showNeverShow = self.playerLevel and self.playerLevel >= kRookieAllowedLevel
        ...
        elseif (not pastWarning or pastWarning == 1) and not Client.GetOptionBoolean("never_show_roa", false) 
            and MainMenu_GetSelectedIsRookieOnly() and Client.GetLevel() >= kRookieAllowedLevel 
        then
...
function GUIMainMenu:CreateFilterForm()
    ...
    local oldRankedOnly = string.ToBoolean(Client.GetOptionString("filter_ranked_only", "true"))
    self.filterRankedOnly:SetValue(Client.GetOptionString("filter_show_unranked", tostring(not oldRankedOnly)))
...
function GUIMainMenu:CreateServerListWindow()
    ...
    local rowNames = { { Locale.ResolveString("SERVERBROWSER_FAVORITE"), Locale.ResolveString("SERVERBROWSER_PRIVATE"), Locale.ResolveString("SERVERBROWSER_NAME"), Locale.ResolveString("SERVERBROWSER_GAME"), Locale.ResolveString("SERVERBROWSER_MAP"), Locale.ResolveString("SERVERBROWSER_PLAYERS"), Locale.ResolveString("SERVERBROWSER_PERF"), Locale.ResolveString("SERVERBROWSER_PING") } }
    
    local serverList = self.serverList

    --Default sorting
    serverList:SetComparator(SortByFavorite)
    ...
    local entryCallbacks = {
        ...
        { OnClick = function() UpdateSortOrder(7) serverList:SetComparator(SortByPerformance) end },
    ...
    if self:GetIsVisible() then
        ...
        if self.currentAlertText ~= alertText then
            ...
            if self.currentAlertText then
                ...
                local wrap = WordWrap( self.alertText.text, self.currentAlertText, 0, 500 )
...
function GUIMainMenu:HideMenu()
    ...
    if self.newsScript and self.newsScript.isVisible then
...
function GUIMainMenu:PopupIsVisible()
    return 
        ...
        ( self.changelog and self.changelog:GetIsVisible() )
end
...
function GUIMainMenu:MaybeAddNewItemPopup()
    ...
    if new then
        ...
        if new == 906 then 
            -- Play music for the new player progress patch
...
function GUIMainMenu:MaybeCreateModWarningWindow()
    ...
    hint:SetTextClipped( true, GUIScaleWidth(300), GUIScaleHeight(560) )
...
function GUIMainMenu:ActivatePlayWindow(playNow, gamemode)
...
function GUIMainMenu:CreateRookieOnlyNagWindow()
    ...
    hint:SetTextClipped( true, GUIScaleWidth(400), GUIScaleHeight(560) )
...
function GUIMainMenu:StartTutorial()
    ...
    --the open animation of the newsScript is still playing so we have to set this falg manually
